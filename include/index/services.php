<section class="i-section services" id="services">
    <div class="container">
        <div class="b-article ">
        <h2>Наши услуги</h2>
            <div class="b-article__gridContainer d-grid">
                <div class="b-article__services_link"><a href="#outsourcingServers" class="b-service-btn">Аутсорсинг серверов</a></div>
                <div class="b-article__services_link"><a href="#virtualisation" class="b-service-btn">Виртуализация</a></div>
                <div class="b-article__services_link"><a href="#1CSupport" class="b-service-btn">Сопровождение программных продуктов 1С</a></div>
                <div class="b-article__services_link"><a href="#PCSupport" class="b-service-btn">Обслуживание компьютеров</a></div>
                <div class="b-article__services_link"><a href="#setUpNetwork" class="b-service-btn">Настройка сетевого оборудования</a></div>
                <div class="b-article__services_link"><a href="#phoneSupport" class="b-service-btn">Обслуживание телефонии</a></div>
                <div class="b-article__services_link"><a href="#businessAutomatisation" class="b-service-btn">Автоматизация бизнеса</a></div>
                <div class="secondary-primary-btn d-flex mt-2 align-items-center justify-content-center"><a href="#">ПОЗВОНИТЬ</a></div>
                <div class="b-article__services_link"><a href="#video" class="b-service-btn">Видеонаблюдение</a></div>
            </div>
        </div>
    </div>
</section>